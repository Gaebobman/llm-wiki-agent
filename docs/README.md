...
- Telegram 명령은 `/status`, `/scan`, `/queue`, `/ingest`를 우선 제공한다.
- qmd는 현재 stub 인터페이스만 두고, 실제 검색 라우팅은 후속 Phase에서 연결한다.
...
- 바이너리 문서는 현재 placeholder source note를 생성한다.
- `wiki/index.md`와 `wiki/log.md`를 갱신한다.
- manifest와 queue 상태를 `ingested`/`done`으로 바꾼다.
- qmd refresh stub을 호출한다.
...
`agent.qmd_client`는 Phase 6에서 실제 qmd CLI/API로 교체하기 위한 인터페이스다.

- `refresh_changed(settings)`는 현재 stub 성공 응답을 반환한다.
- `search(settings, query, mode)`는 빈 검색 결과와 stub 메시지를 반환한다.
- ingest worker는 source note 생성 후 refresh stub을 호출한다.
